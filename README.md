...
✅ Feature-complete: upload/scan a receipt (or skip and enter items by hand), split it either as a named group (per-item/per-unit assignment, custom shares) or solo (just your own fraction of each item), and get a shareable summary, all offline-capable, in English or Italian, with a light or dark theme.
