 ...
 * items: trims the header/footer, merges standalone "N x price" quantity
 * lines into their adjacent item line, extracts name/price/quantity per
 * line, merges per-item discounts and separates whole-receipt discounts,
 * then groups identical repeated lines into one item with a summed
 * quantity. This is intentionally best-effort — store formats vary too much
 * to guarantee a perfect parse; the UI's manual-correction tools are the
