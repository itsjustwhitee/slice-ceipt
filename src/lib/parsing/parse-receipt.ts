 ...
 * the running-total line, merges weight rows/"Cad" quantity-disclosure
 * lines/wrapped names/bare-quantity lines into their item line, extracts
 * name/price/quantity, merges
...
export function parseReceiptText(rawText: string): ParsedItem[] {
	const lines = normalizeRunningTotalLines(mergeCadQuantityLines(mergeWeightRows(rawText.split('\n'))));
